	...
	SpriteBatch batch;
	  ...
	  private void moveMissile(){
	    if( Gdx.input.isKeyPressed(Input.Keys.SPACE) && !attack ){
	      attack = true;
	      // ALTERADO
	      yMissile = posY  + ship.getHeight() / 2 - 12;
	    }
	  ...
	  private void moveEnemies(){
	    ...
	    for( Iterator<Rectangle> iter = enemies.iterator(); iter.hasNext(); ){
	      ...
	      if( collide(enemy.x, enemy.y, enemy.width, enemy.height, xMissile, yMissile, missile.getWidth(), missile.getHeight()) && attack ){
	        ...
	        iter.remove();
	      // Colisão com a nave
	      }else if( collide(enemy.x, enemy.y, enemy.width, enemy.height, posX, posY, ship.getWidth(), ship.getHeight()) && !gameover ){
	        --power;
	        if( power <= 0 ){
	          gameover = true;
	        }
